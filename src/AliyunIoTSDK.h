...
private:
  ...
  // 子设备信息
  static char childClientId[256];
  static char childMqttUsername[100];
  static char childDomain[150];
  static char childMqttPwd[256];
...
public:
  ...
  // 子设备 相关topic
  static char ALINK_TOPIC_LOGIN[150];
  static char ALINK_TOPIC_CHILD_PROP_POST[150];
  ...
  static void addChildDevice(const char *_productKey,
                             const char *_deviceName,
                             const char *_deviceSecret,
                             const char *_region);

  static void subTopic(const char *topic);
  /**
   * 发送数据
   * @param param 字符串形式的json 数据，例如 {"${key}":"${value}"}
   */
  static void send(const char *param);
  static void childSend(const char *param);
  /**
   * 发送 float 格式数据
   * @param key 数据的 key
   * @param number 数据的值
   ...
   * @param param 登录字符串
   */
  static void login(const char *param);
