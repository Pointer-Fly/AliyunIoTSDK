...
static const char *child_deviceName = NULL;
static const char *child_productKey = NULL;
static const char *child_deviceSecret = NULL;
static const char *child_region = NULL;
...
char AliyunIoTSDK::ALINK_TOPIC_LOGIN[150] = "";
char AliyunIoTSDK::ALINK_TOPIC_CHILD_PROP_POST[150] = "";

char AliyunIoTSDK::childClientId[256] = "";
char AliyunIoTSDK::childMqttUsername[100] = "";
char AliyunIoTSDK::childMqttPwd[256] = "";
char AliyunIoTSDK::childDomain[150] = "";
...
static void callback(char *topic, byte *payload, unsigned int length)
{
    ...
    else if (strstr(topic, "/sys/a1NTud4MOfM/Lora-ESP32-GatewayDevice/thing/service/property/set"))
    {
        StaticJsonDocument<200> doc;
        DeserializationError error = deserializeJson(doc, payload); //反序列化JSON数据
                         ...
                         const char *_region)
{
    ...
    sprintf(ALINK_TOPIC_LOGIN, "/ext/session/%s/%s/combine/login", productKey, deviceName);
...
void AliyunIoTSDK::addChildDevice(const char *_productKey,
                                  const char *_child_deviceName,
                                  const char *_child_deviceSecret,
                                  const char *_child_region = NULL)
{
    child_productKey = _productKey;
    child_deviceName = _child_deviceName;
    child_deviceSecret = _child_deviceSecret;
    if (_child_region == NULL)
    {
        child_region = region;
    }
    else
    {
        child_region = _child_region;
    }

    sprintf(ALINK_TOPIC_CHILD_PROP_POST, "/sys/%s/%s/thing/event/property/post", child_productKey, child_deviceName);
}

void AliyunIoTSDK::subTopic(const char *topic)
{
...
void AliyunIoTSDK::send(const char *param)
{
    ...
    // Serial.print("[parent]: publish:0 成功:");
    // Serial.println(d);
}
void AliyunIoTSDK::childSend(const char *param)
{
    char jsonBuf[1024];
    sprintf(jsonBuf, ALINK_BODY_FORMAT, param);
    // Serial.println(jsonBuf);
    boolean d = client->publish(ALINK_TOPIC_CHILD_PROP_POST, jsonBuf);
    // Serial.print("[child]: publish:0 成功:");
    // Serial.println(d);
}
...
void AliyunIoTSDK::login(const char *param)
{
    char jsonBuf[1024];
    sprintf(jsonBuf, ALINK_BODY_FORMAT, param);
    Serial.println(jsonBuf);
    boolean d = client->publish(ALINK_TOPIC_LOGIN, jsonBuf);
    // Serial.print("[login]:成功:");
    Serial.println(d);
}
